...
![](https://api.cuiliangblog.cn/v1/public/imgProxy/?url=https://cdn.nlark.com/yuque/0/2025/png/2308212/1756542945451-95ee4821-49ea-4e9e-ad96-2dbdeed5b81c.png)

之后，将文档片段通过文本嵌入模型组件，转换为向量，通过向量数据库组件，保存到向量数据库，每个向量通常还会绑定原文内容、文档ID、来源等元数据信息，便于后面数据检索。

![](https://api.cuiliangblog.cn/v1/public/imgProxy/?url=https://cdn.nlark.com/yuque/0/2025/png/2308212/1756542945422-76451b2c-8cc6-442e-a255-bb62be15e12a.png)
...
![](https://api.cuiliangblog.cn/v1/public/imgProxy/?url=https://cdn.nlark.com/yuque/0/2025/png/2308212/1756542945447-c453aec4-5b75-4578-844d-cd441b3f4780.png)
...
![](https://api.cuiliangblog.cn/v1/public/imgProxy/?url=https://cdn.nlark.com/yuque/0/2025/png/2308212/1757166352765-4eaa57dc-88f6-4335-bd25-de9766fa940d.png)
...
```python title="打印文档内容" icon="python"
文档数量：92
文档内容：什么是 RAG
...
```console title="使用示例" icon="terminal"
共解析到 9 个文档
...
```console title="对已按标题分割的文档片段进行二次递归分割，确保每个片段不超过指定大小" icon="terminal"
按标题分割文档数量：15
...
![](https://api.cuiliangblog.cn/v1/public/imgProxy/?url=https://cdn.nlark.com/yuque/0/2025/png/2308212/1757236408182-c88327fa-5af1-4291-b089-890ed73b172f.png)

### 5.3 RedisVectorStore数据检索
...
```console title="将查询语句向量化，并在Redis中做相似度检索" icon="terminal"
=== 查询结果 ===
结果 1:
内容: 我喜欢用苹果手机
元数据: {'segment_id': '3'}
...
![](https://api.cuiliangblog.cn/v1/public/imgProxy/?url=https://cdn.nlark.com/yuque/0/2025/png/2308212/1757236828615-d9517f9e-e8d6-41b9-9c7a-6254fe65d8b0.png)

该默认提示词为英文版，在使用时需要进行汉化，否则返回的查询文本将全部为英文，导致检索效果下降。

![](https://api.cuiliangblog.cn/v1/public/imgProxy/?url=https://cdn.nlark.com/yuque/0/2025/png/2308212/1757236828633-c3189f40-adab-4ac3-a029-14189b246b7c.png)
...
```python title="MultiQueryRetriever使用" icon="python"
...
![](https://api.cuiliangblog.cn/v1/public/imgProxy/?url=https://cdn.nlark.com/yuque/0/2025/png/2308212/1757241582153-2a18528e-5484-46ea-88d7-b3d694f474e0.png)
...
```python title="安装浏览器中文依赖" icon="python"
...
```console title="文档处理" icon="terminal"
# cat faq.json
[
  {
...
```python title="文档数据向量化" icon="python"
import json
from langchain_ollama import OllamaEmbeddings
from langchain_redis import RedisConfig, RedisVectorStore
...
![](https://api.cuiliangblog.cn/v1/public/imgProxy/?url=https://cdn.nlark.com/yuque/0/2025/png/2308212/1757249415535-7c5e69e7-3481-4f91-9d96-da0314ba8d1c.png)
