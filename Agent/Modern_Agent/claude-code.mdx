...
先看完整的架构全景图，再分层展开：
~~~text title="Agent 查询循环全景" icon="file"
用户输入
↓
...
~~~text title="Query Loop 控制流" icon="file"
while (true) {
  1. 把当前消息发给模型 → 拿到模型的回复
...
~~~ts title="Agent 工具输入结构" icon="file"
type AgentToolInput = {
  prompt: string              // 任务描述
...
~~~text title="Agent 通信模式回答" icon="file"
面试时可以从三个层面回答：
一.架构选型：
