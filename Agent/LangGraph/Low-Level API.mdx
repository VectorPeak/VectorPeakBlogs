...
![](https://api.cuiliangblog.cn/v1/public/imgProxy/?url=https://cdn.nlark.com/yuque/0/2025/png/2308212/1758504633150-3238595b-a2d3-4a04-b735-ff11634f3d7d.png)
...
```python title="手动创建图流程" icon="python"
...
```console title="打印图的可视化结构" icon="terminal"
...
```python title="打印图的可视化结构" icon="python"
# 打印图的可视化结构
...
![](https://api.cuiliangblog.cn/v1/public/imgProxy/?url=https://cdn.nlark.com/yuque/0/2025/png/2308212/1758596247026-6963b93b-7827-407d-a058-11b566ddace9.png)

### 2.6 图对象运行

当我们通过`builder.compile()`方法编译图后，编译后的`graph`对象提供了`invoke`方法，该方法用于启动图的执行。在图执行前我们需要通过`invoke`方法传递一个初始状态，这个状态作为图执行的起始输入：

```python title="图对象运行" icon="python"
...
```console title="调用graph对象的invoke方法，传入初始状态，执行图计算流程" icon="terminal"
加法节点收到的初始值:{'x': 5}
减法节点收到的初始值:{'x': 6}
最后的结果是:{'x': 4}
```
...
```python title="Pydantic基本使用" icon="python"
...
```console title="类型错误会报错" icon="terminal"
1
default
……
...
```python title="Pydantic应用于StateGraph" icon="python"
from langgraph.constants import START, END
from langgraph.graph import StateGraph
from pydantic import BaseModel
...
![](https://api.cuiliangblog.cn/v1/public/imgProxy/?url=https://cdn.nlark.com/yuque/0/2025/png/2308212/1758596659797-173bdfcd-1a80-4964-9588-f69c9bc240dc.png)

代码如下：

```python title="条件判断" icon="python"
...
```console title="测试用例：输入奇数3" icon="terminal"
...
![](https://api.cuiliangblog.cn/v1/public/imgProxy/?url=https://cdn.nlark.com/yuque/0/2025/png/2308212/1758596729546-66dd1eea-6789-432b-921f-b819d626a376.png)

代码如下

```python title="循环语句" icon="python"
from langgraph.constants import START, END
from langgraph.graph import StateGraph
from loguru import logger
from pydantic import BaseModel
...
```console title="初始化循环并执行，直到满足结束条件" icon="terminal"
...
![](https://api.cuiliangblog.cn/v1/public/imgProxy/?url=https://cdn.nlark.com/yuque/0/2025/png/2308212/1758597109547-34741ad6-7757-4216-a80d-098c74682c45.png)

代码如下

```python title="判断循环复合图" icon="python"
from loguru import logger
from pydantic import BaseModel
from typing import Optional
...
```console title="测试用例2：从奇数开始，直接结束流程" icon="terminal"
...
```console title="执行图并打印结果" icon="terminal"
...
![](https://api.cuiliangblog.cn/v1/public/imgProxy/?url=https://cdn.nlark.com/yuque/0/2025/png/2308212/1759501842750-67d188d7-e562-4cda-8c7f-fdf31560767d.png)
...
```python title="最简单的聊天机器人" icon="python"
from typing import TypedDict, Annotated
from langchain_ollama import ChatOllama
from langgraph.constants import START, END
from langgraph.graph import add_messages, StateGraph
...
```console title="编译图结构，并绘制可视化图表" icon="terminal"
对不起，我无法获取北京的天气信息，可以查看北京气象局或使用天气预报应用。
```

整体流程如下

![](https://api.cuiliangblog.cn/v1/public/imgProxy/?url=https://cdn.nlark.com/yuque/0/2025/png/2308212/1759842510576-3479908c-4f79-4a18-aa2f-2eddbaeeb123.png)

### 8.2 添加提示词与工具

定义工具函数

```python title="添加提示词与工具" icon="python"
import json
import os
import httpx
import dotenv
...
```python title="加载环境变量配置" icon="python"
from typing import TypedDict, Annotated
...
![](https://api.cuiliangblog.cn/v1/public/imgProxy/?url=https://cdn.nlark.com/yuque/0/2025/png/2308212/1759896219304-2766d022-16a2-429d-b1f3-1a91f59861d8.png)

### 8.3 添加 HITL 环节

代码如下

```python title="添加 HITL 环节" icon="python"
from typing import TypedDict, Annotated
import json
...
```console title="测试运行" icon="terminal"
【用户】北京天气怎么样
...
```python title="添加时间回溯" icon="python"
from typing import TypedDict, Annotated
...
```console title="从第二个检查点恢复并注入新问题" icon="terminal"
==================================================
第一轮对话：询问北京天气
==================================================
