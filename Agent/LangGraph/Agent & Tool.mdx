...
```python title="安装 LangGraph" icon="python"
...
```python title="接入自定义工具函数" icon="python"
import json
import os
import httpx
import dotenv
...
```console title="- custom : 自定义输出。主要是可以在工具内部使用get_stream_writer获取输入流，添加自定义的内容" icon="terminal"
...
![画板](https://api.cuiliangblog.cn/v1/public/imgProxy/?url=https://cdn.nlark.com/yuque/0/2025/jpeg/2308212/1757990081585-09098a21-7c10-40ba-8c10-658a56d925be.jpeg)
...
```python title="添加多个工具函数" icon="python"
import json
import os
import httpx
import dotenv
...
```python title="工具并联调用" icon="python"
from langchain_ollama import ChatOllama
...
```console title="输出完整响应结果和最终回答内容" icon="terminal"
...
```python title="工具串联调用" icon="python"
from langchain_ollama import ChatOllama
from tools import get_weather, write_file
from langgraph.prebuilt import create_react_agent
...
```python title="创建带搜索功能的Agent" icon="python"
import os
import dotenv
from langchain_ollama import ChatOllama
