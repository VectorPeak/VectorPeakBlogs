...
```python title="创建FastMCP实例，用于启动天气服务器SSE服务" icon="python"
# uv run server.py
...
```python title="创建 mcp配置文件" icon="python"
{
  "mcpServers": {
    "weather": {
...
```python title="LangGraph 客户端" icon="python"
import asyncio
import json
...
```python title="添加天气助手MCP工具" icon="python"
{
  "mcpServers": {
    "get_weather": {
...
```python title="更新提示词" icon="python"
prompt = """
